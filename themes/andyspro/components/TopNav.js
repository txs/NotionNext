...
const TopNav = (props) => {
  ...
  // 监听滚动
  useEffect(() => {
    if (CONFIG_ANDYSPRO.NAV_TYPE === 'autoCollapse') {
      // scrollTrigger()
      window.addEventListener('scroll', scrollTrigger)
    }
    return () => {
      CONFIG_ANDYSPRO.NAV_TYPE === 'autoCollapse' && window.removeEventListener('scroll', scrollTrigger)
    }
  }, [])
  ...
  return (<div id='top-nav' className='z-40'>
    ...
    {/* 导航栏 */}
    <div id='sticky-nav' className={`${CONFIG_ANDYSPRO.NAV_TYPE !== 'normal' ? 'sm:local md:fixed bg-white' : ' bg-none -mb-10'} animate__animated animate__fadeIn dark:bg-black dark:bg-opacity-50 dark:text-gray-200  bg-opacity-80 text-black w-full top-0 z-20 transform duration-200 font-sans`}>
